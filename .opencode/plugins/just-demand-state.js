...
const CROSS_SENTENCE_NEAR_MISS_PATTERNS = [
  ...
  /\b(暂时|先|还要|还需要)\s*(?:确认|核对|确认一下|核对一下|再确认|再核对)\b/i,
  /(?:还不能|暂不|暂时不|不打算)(?:完成|收尾|结束).*(?:先|还要|需要).*(?:确认|核对)/i,
  /(?:暂时)?不打算(?:完成|收尾|结束)/i,
]
...
const buildControllerDecision = (text, reminderState) => {
  ...
  if (activeTask) {
    ...
    // Small work always stays inline; larger work is handled by the selective
    // dispatch policy rather than by a runtime routing block.
    if (textLooksLikeSmallWork(text)) {
      return {
        phase: CONTROLLER_PHASE.execute,
        action: CONTROLLER_ACTION.allow,
        reason_code: "small_work_bypass",
        rewrite: null,
      }
    }
    ...
    if (
      taskLooksLikeLongContextExecutionCandidate(activeTask, text)
      || (textLooksLikeCodeInvestigationIntent(text) && !textLooksLikeNeutralAnalysis(text))
    ) {
      return {
        phase: CONTROLLER_PHASE.execute,
        action: CONTROLLER_ACTION.allow,
        reason_code: "main_session_execution",
        rewrite: null,
      }
    }
  }
  ...
  switch (type) {
    ...
    case "clarify":
      return [
        ...
        "- Use subagents only when all six eligibility gates and all three net-benefit questions pass.",
      ]
    case "premise_check":
      return [
    ...
    case "execution_needed":
      return [
        "- This lifecycle transition is not available from the current execution phase.",
        "- Continue the ready task in the main session or dispatch selectively, then verify before completion.",
      ]
...
const blockExecutionNeeded = (text, reminderState) => {
  ...
  return [
    ...
    "- This lifecycle transition is not available from the current task phase.",
    "- Continue implementation or verification as indicated by the workflow-state banner.",
    "- Complete the task only through `complete-verification` after checks pass.",
    "",
    "Original response:",
    quotedText,
  ].join("\n")
...
export default async ({ directory } = {}) => {
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      ...
      const controllerStatus = controllerDecision.action === "block" ? "blocked"
        : controllerDecision.action === "remind" ? "applied_with_reminder"
        : "applied"
      appendDebugSessionAudit(workflowDirectory, sessionID, "visible", {
        source: "chat.message",
        task_id: activeTaskId || "",
        status: "observed",
        reason: controllerDecision.reason_code,
        phase: controllerDecision.phase,
        action: controllerDecision.action,
      ...
      appendDebugSessionAudit(workflowDirectory, sessionID, "main-agent", {
        source: "chat.message",
        task_id: activeTaskId || "",
        status: controllerStatus,
        reason: controllerDecision.reason_code,
